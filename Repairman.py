...
class Repairman:
    ...
    def agregar_columna_code_corregida(self):
        ...
        self.df_a['name'] = self.df_a['name'].apply(self.repair_name)
    ...
    def get_full_code(self,code):
        ...
        for sub_code in codes:
            ...
            if '-' in sub_code:
                ...
                full_codes.extend(str(i) for i in range(start, end+1))
            else:
                full_codes.append(sub_code)

        if not full_codes:
            for i in range(1, 7):
                for j in range(len(code) - i + 1):
                    sub_code = code[j:j+i]
                    if sub_code in self.df_b['code'].values:
                        full_codes.append(sub_code)

        return full_codes if full_codes else code
    def train(self):
        ...
        self.df_d['name'] = self.df_d['name'].str.upper().str.normalize('NFKD').str.encode('ascii', errors='ignore').str.decode('utf-8')

        # Entrenar el vectorizador con las formas de decir las enfermedades en df_b
        print(f"\nEntrenando el vectorizador...\n")
        print('datos: ', 'len de df_a: ', len(self.df_a['name']), 'len de df_b: ', len(self.df_d['name']))
        X_train = self.vectorizer.fit_transform(self.df_d['name'])
        

        # Entrenar el clasificador con los vectores de las formas de decir las enfermedades y sus correspondientes códigos
        y_train = self.df_d['code']
    ...
    def prepare_df_a(self):
        ...
        # Create a copy of df_a
        df_a_copy = self.df_a.copy()
        
        # Apply the change in the 'name' column of the copied dataframe
        df_a_copy['name'] = df_a_copy['name'].replace(change, regex=True)
    ...
    def predict_codes(self, df_a):
        ...
        code_to_name = self.df_d.set_index('code')['name'].to_dict()
        ...
        # Aplicar get_full_code a cada valor en la columna 'code'
        df_a['code'] = df_a['code'].apply(self.get_full_code)
